...
$(package)_patches = remove_pthread_stubs.patch silent_xorg_macros_probe.patch
...
$(package)_cflags += -Wno-unused-result -Wno-discarded-qualifiers
endef

define $(package)_preprocess_cmds
  cp -f $(BASEDIR)/config.guess $(BASEDIR)/config.sub build-aux && \
  patch -p1 -i $($(package)_patch_dir)/remove_pthread_stubs.patch && \
  patch -p1 -i $($(package)_patch_dir)/silent_xorg_macros_probe.patch
endef
