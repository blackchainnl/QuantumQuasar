...
**The `configure` script by default will ignore the depends output.** In
...
Blackcoin V4 builds are 64-bit only. The inherited depends build system may
still contain low-level 32-bit triplet plumbing for upstream compatibility, but
32-bit host triplets are rejected before dependency compilation begins.
...
Common linux dependencies:
    ...
    sudo apt-get install make automake cmake curl g++ libtool binutils bsdmainutils pkg-config python3 patch bison
