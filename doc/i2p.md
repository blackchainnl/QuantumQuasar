# I2P support in Blackcoin

It is possible to run Blackcoin as an
...
## Run Blackcoin with an I2P router (proxy)

A running I2P router (proxy) is required with the [SAM](https://geti2p.net/en/docs/api/samv3)
application bridge enabled. The following routers are recommended for use with Blackcoin:
...
I2P support was added to Blackcoin in version 22.0 and there may be fewer I2P
...
The first time Blackcoin connects to the I2P router, it automatically
...
`i2p_private_key` in the Blackcoin data directory.  The persistent I2P
...
## Fetching I2P-related information from Blackcoin

There are several ways to see your I2P address in Blackcoin if accepting
...
You can use the `getnodeaddresses` RPC to fetch a number of I2P peers known to your node; run `blackcoin-cli help getnodeaddresses` for details.

## Compatibility

Blackcoin uses the [SAM v3.1](https://geti2p.net/en/docs/api/samv3) protocol
to connect to the I2P network. Any I2P router that supports it can be used.

## Ports in I2P and Blackcoin
...
To allow future upgrades to newer versions of SAM, Blackcoin sets its
listening port to 0 when listening for incoming I2P connections and advertises
its own I2P address with port 0. Furthermore, it will not attempt to connect to
I2P addresses with a non-zero port number because with SAM v3.1 the destination
port (`TO_PORT`) is always set to 0 and is not in the control of Blackcoin.
