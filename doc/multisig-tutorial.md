...
Currently, it is possible to create a multisig wallet using Blackcoin only.
...
At the time of writing, there is no way to extract a specific path from wallets in Blackcoin. For this, an external signer/xpub can be used.
...
for ((n=1;n<=3;n++))
do
 ./src/blackcoin-cli -signet createwallet "participant_${n}"
done
```

Extract the xpub of each wallet. To do this, the `listdescriptors` RPC is used. By default, Blackcoin single-sig wallets are created using path `m/44'/1'/0'` for PKH, `m/84'/1'/0'` for WPKH, `m/49'/1'/0'` for P2WPKH-nested-in-P2SH and `m/86'/1'/0'` for P2TR based accounts. Each of them uses the chain 0 for external addresses and chain 1 for internal ones, as shown in the example below.
...
All RPCs in Blackcoin will include the checksum in their output.
...
```bash
./src/blackcoin-cli -signet -named createwallet wallet_name="multisig_wallet_01" disable_private_keys=true blank=true
