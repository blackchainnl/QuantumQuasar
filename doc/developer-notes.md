...
- [Development guidelines](#development-guidelines)
    - [General Blackcoin](#general-bitcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Blackcoin uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
configuration options and toggled while blackcoind is running with the `logging`
RPC.  For instance, launching blackcoind with `-debug` or `-debug=1` will turn on
...
Blackcoin is a multi-threaded application, and deadlocks or other
...
i.e. `CPPFLAGS="-DDEBUG_LOCKCONTENTION"`, then build and run blackcoind.
...
Blackcoin can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`blackcoind`)](https://doxygen.bitcoincore.org/blackcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
  : Started from `main()` in `blackcoind.cpp`. Responsible for starting up and
...
However, in open source software such as Blackcoin, where everyone uses
...
pay attention to for reviewers of Blackcoin code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Blackcoin uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of Blackcoin, in which case
...
a Blackcoin subtree, so that they can be integrated quickly. Cosmetic changes
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Blackcoin, the wallet was always
