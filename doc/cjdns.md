# CJDNS support in Blackcoin

It is possible to run Blackcoin over CJDNS, an encrypted IPv6 network that
...
done outside of Blackcoin, similarly to a VPN (either in the host/OS or on
...
before it will work with your Blackcoin node. This is described in steps
...
## Run Blackcoin with CJDNS

Once you are connected to the CJDNS network, the following Blackcoin
...
When enabled, this option tells Blackcoin that it is running in an
...
IPv6 local network. This helps Blackcoin perform better address management:
...
CJDNS support was added to Blackcoin in version 23.0 and there may be fewer
...
## CJDNS-related information in Blackcoin

There are several ways to see your CJDNS address in Blackcoin:
...
You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `blackcoin-cli help getnodeaddresses` for details.
