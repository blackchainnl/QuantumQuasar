UNIX BUILD NOTES
====================
Some notes on how to build Blackcoin in Unix.
...
memory available when compiling Blackcoin. On systems with less, gcc can be
...
To build Blackcoin without wallet, see [*Disable-wallet mode*](#disable-wallet-mode)
...
...
to: /path/to/blackcoin/depends/x86_64-pc-linux-gnu
```
and configure using the following:
```bash
export BDB_PREFIX="/path/to/blackcoin/depends/x86_64-pc-linux-gnu"
...
Disable-wallet mode
--------------------
When the intention is to only run a P2P node, without a wallet, Blackcoin can
