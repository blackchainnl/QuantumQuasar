# User-space, Statically Defined Tracing (USDT) for Blackcoin

Blackcoin includes statically defined tracepoints to allow for more
...
**Note**: Blackcoin uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Blackcoin
...
To list probes in Blackcoin, use `info probes` in `gdb`:
...
The `readelf` tool can be used to display the USDT tracepoints in Blackcoin.
