...
The Blackcoin project operates an open contributor model where anyone is
welcome to contribute towards development in the form of peer review, testing
and patches. This document explains the practical process and guidelines for
contributing.

First, in terms of structure, there is no particular concept of "Blackcoin
...
Before you start contributing, familiarize yourself with the Blackcoin build
system and tests. Refer to the documentation in the repository on how to build
Blackcoin and how to run the unit tests, functional tests, and fuzz tests.

There are many open issues of varying difficulty waiting to be fixed.
If you're looking for somewhere to start contributing, check the open issues in
this repository. Some older upstream issues may no longer apply to this fork, so
leave a comment before spending substantial time on inherited work.

Protocol, wallet, and consensus changes should be discussed in issues or draft
...
requests. Complicated or controversial consensus, wallet, staking, or P2P
changes should start as a design issue or draft pull request before a patch set.
...
Open pull requests against this repository. GUI-only work usually lives under
`src/qt`, but GUI changes that touch `src/interfaces`, wallet code, node code, or
the build system require broader review because they affect more than the Qt
front end.
...
the pull request affects. Valid areas as:
  ...
  - `qt` or `gui` for changes to the Qt GUI
...
PR description may be included in the commit message when the PR is merged and
copied into downstream forks. Make any username mentions in a subsequent comment
to the PR.

### Translation changes

Translation updates should be coordinated with the current Qt translation
workflow instead of broad manual catalog edits.
...
The following applies to code changes to the Blackcoin project (and related
projects such as libsecp256k1), and is not to be confused with overall Blackcoin
Network Protocol consensus changes.

Whether a pull request is merged into Blackcoin rests with the project merge
maintainers.
