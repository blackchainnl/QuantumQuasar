...
CAmount GetDustThreshold(const CTxOut& txout, const CFeeRate& dustRelayFeeIn)
{
    ...
    if (IsQuantumMigrationScript(txout.scriptPubKey)) {
        ...
        nSize += GetVirtualTransactionSize(non_witness * WITNESS_SCALE_FACTOR + witness, 0, 0);
    } else if (IsQuantumColdStakeScript(txout.scriptPubKey)) {
        const int64_t non_witness = 32 + 4 + 1 + 4;
...
bool IsStandard(const CScript& scriptPubKey, const std::optional<unsigned>& max_datacarrier_bytes, TxoutType& whichType, const bool witnessEnabled)
{
    ...
    if (whichType == TxoutType::NONSTANDARD) {
        ...
        if (m < 1 || m > n)
            return false;
    } else if (whichType == TxoutType::NULL_DATA || whichType == TxoutType::RGB_COMMITMENT) {
        const bool is_b7_attestation = whichType == TxoutType::NULL_DATA && Consensus::IsDemurrageAttestationScript(scriptPubKey);
        if (!max_datacarrier_bytes || (!is_b7_attestation && scriptPubKey.size() > *max_datacarrier_bytes)) {
            return false;
        }
    } else if (!witnessEnabled && (whichType == TxoutType::WITNESS_V0_SCRIPTHASH || whichType == TxoutType::WITNESS_V0_KEYHASH || whichType == TxoutType::WITNESS_V1_TAPROOT || whichType == TxoutType::EUTXO_COMMITMENT || whichType == TxoutType::WITNESS_UNKNOWN)) {
        return false;
    }
    return true;
}
...
bool IsStandardTx(const CTransaction& tx, const std::optional<unsigned>& max_datacarrier_bytes, bool permit_bare_multisig, const CFeeRate& dust_relay_fee, std::string& reason, const bool witnessEnabled)
{
    ...
    for (const CTxOut& txout : tx.vout) {
        ...
        if (whichType == TxoutType::NULL_DATA || whichType == TxoutType::RGB_COMMITMENT)
            nDataOut++;
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
        ...
        if (whichType == TxoutType::NONSTANDARD) {
            return false;
                   ...
                   !IsQuantumMigrationWitnessProgram(vSolutions[0][0], vSolutions[1]) &&
                   !IsQuantumColdStakeWitnessProgram(vSolutions[0][0], vSolutions[1])) {
            return false;
        } else if (whichType == TxoutType::EUTXO_COMMITMENT && !IsEUTXOWitnessProgram(vSolutions[0][0], vSolutions[1])) {
            return false;
...
bool IsWitnessStandard(const CTransaction& tx, const CCoinsViewCache& mapInputs)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++)
    {
        ...
        if (IsQuantumMigrationWitnessProgram(witnessversion, witnessprogram)) {
            if (p2sh) return false;
            const auto& stack = tx.vin[i].scriptWitness.stack;
            if (stack.size() != 2) return false;
            if (stack[0].size() != ML_DSA::SIGNATURE_BYTES) return false;
            if (stack[1].size() != ML_DSA::PUBLICKEY_BYTES) return false;
        }

        if (IsQuantumColdStakeWitnessProgram(witnessversion, witnessprogram)) {
            if (p2sh) return false;
            const auto& stack = tx.vin[i].scriptWitness.stack;
            ...
            if (!stack[3].empty() && !(stack[3].size() == 1 && stack[3][0] == 1)) return false;
        }

        if (IsEUTXOWitnessProgram(witnessversion, witnessprogram)) {
            if (p2sh) return false;
            const auto& stack = tx.vin[i].scriptWitness.stack;
            if (stack.size() < 3) return false;
            if (stack[stack.size() - 3].size() > V4_MAX_SCRIPT_ELEMENT_SIZE) return false;
            if (stack[stack.size() - 2].size() > V4_MAX_SCRIPT_ELEMENT_SIZE) return false;
            if (stack.back().size() > V4_MAX_SCRIPT_ELEMENT_SIZE) return false;
        }
