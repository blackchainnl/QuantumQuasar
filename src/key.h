...
private:
    /** Internal data container for private key material (raw 32-byte secp256k1 secret). */
    using KeyType = std::array<unsigned char, 32>;

    //! Whether the public key corresponding to this private key is (to be) compressed.
    bool fCompressed{false};

    //! The actual length of the valid keydata.
    ...
    {
        if (!keydata) {
            keydata = make_secure_unique<KeyType>();
            keydata->fill(0); // never leave the secret buffer uninitialized
        }
    }
...
public:
    ...
    void Set(const T pbegin, const T pend, bool fCompressedIn)
    {
        ...
        memcpy(key_bytes.data(), &pbegin[0], key_bytes.size());
        if (Check(key_bytes.data())) {
            MakeKeyData();
            keydata->fill(0);
            memcpy(keydata->data(), key_bytes.data(), key_bytes.size());
    ...
    unsigned int size() const { return keydata ? key_size : 0; }
