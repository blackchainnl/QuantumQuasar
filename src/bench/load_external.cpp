...
static void LoadExternalBlockFile(benchmark::Bench& bench)
{
    ...
    CDataStream block_stream(SER_NETWORK);
    block_stream << TX_WITH_WITNESS(*blocks.at(1));

    ss << params.MessageStart();
    ss << static_cast<uint32_t>(block_stream.size());
    // We can't use the streaming serialization of a vector-like object because
    // that first writes a compact size.
    ss << MakeByteSpan(block_stream);
