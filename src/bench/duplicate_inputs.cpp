...
static void DuplicateInputs(benchmark::Bench& bench)
{
    ...
    bench.run([&] {
        ...
        if (cvstate.GetRejectReason() != "bad-txns-inputs-duplicate") {
            throw std::runtime_error(strprintf("DuplicateInputs expected bad-txns-inputs-duplicate, got %s: %s", cvstate.GetRejectReason(), cvstate.GetDebugMessage()));
        }
    });
}
