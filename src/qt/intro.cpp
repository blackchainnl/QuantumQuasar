...
bool IsLegacyBlackcoinDefaultDataDirectory(const QString& data_dir)
{
    const QString clean_data_dir = QDir::cleanPath(data_dir);
    const QString clean_default_dir = QDir::cleanPath(GUIUtil::getDefaultDataDirectory());
    QDir parent_dir(clean_default_dir);
    if (!parent_dir.cdUp()) return false;

    const QStringList legacy_defaults{
        QDir::cleanPath(parent_dir.filePath(QStringLiteral("Blackmore"))),
        QDir::cleanPath(parent_dir.filePath(QStringLiteral(".blackmore"))),
    };
    return legacy_defaults.contains(clean_data_dir);
}
...
bool Intro::showIfNeeded(bool& did_show_intro)
{
    ...
    bool using_legacy_default_dir = IsLegacyBlackcoinDefaultDataDirectory(dataDir);
    ...
    if(!fs::exists(GUIUtil::QStringToPath(dataDir)) || gArgs.GetBoolArg("-choosedatadir", DEFAULT_CHOOSE_DATADIR) || settings.value("fReset", false).toBool() || gArgs.GetBoolArg("-resetguisettings", false))
    {
        ...
        using_legacy_default_dir = false;
    }
     ...
     */
    if(dataDir != GUIUtil::getDefaultDataDirectory() && !using_legacy_default_dir) {
