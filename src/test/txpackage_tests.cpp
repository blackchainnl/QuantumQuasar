...
static CAmount GetFeePaid(const CTransaction& input_tx, int input_vout, const CTransaction& spend_tx)
{
    return input_tx.vout.at(input_vout).nValue - spend_tx.GetValueOut();
}
...
BOOST_FIXTURE_TEST_CASE(package_validation_tests, TestChain100Setup)
{
    ...
    const CAmount parent_fee = GetFeePaid(*m_coinbase_txns[0], 0, *tx_parent);
    const CAmount child_fee = GetFeePaid(*tx_parent, 0, *tx_child);
    ...
    BOOST_CHECK(it_parent->second.m_effective_feerate.value() == CFeeRate(parent_fee, GetVirtualTransactionSize(*tx_parent)));
    ...
    BOOST_CHECK(it_child->second.m_effective_feerate.value() == CFeeRate(child_fee, GetVirtualTransactionSize(*tx_child)));
...
BOOST_FIXTURE_TEST_CASE(package_submission_tests, TestChain100Setup)
{
    ...
    // Submit package with parent + child.
    {
        ...
        BOOST_CHECK(it_parent->second.m_effective_feerate == CFeeRate(parent_fee, GetVirtualTransactionSize(*tx_parent)));
        BOOST_CHECK_EQUAL(it_parent->second.m_wtxids_fee_calculations.value().size(), 1);
        BOOST_CHECK_EQUAL(it_parent->second.m_wtxids_fee_calculations.value().front(), tx_parent->GetWitnessHash());
        BOOST_CHECK(it_child != submit_parent_child.m_tx_results.end());
        BOOST_CHECK(it_child->second.m_state.IsValid());
        BOOST_CHECK(it_child->second.m_effective_feerate == CFeeRate(child_fee, GetVirtualTransactionSize(*tx_child)));
        BOOST_CHECK_EQUAL(it_child->second.m_wtxids_fee_calculations.value().size(), 1);
        BOOST_CHECK_EQUAL(it_child->second.m_wtxids_fee_calculations.value().front(), tx_child->GetWitnessHash());
...
BOOST_FIXTURE_TEST_CASE(package_witness_swap_tests, TestChain100Setup)
{
    ...
    mineBlocks(5);
    MockMempoolMinFee(CFeeRate(DEFAULT_MIN_RELAY_TX_FEE * 2));
    LOCK(cs_main);
    ...
    const CAmount mixed_child_fee{COIN - low_fee_amt};
    mtx_mixed_child.vout.emplace_back(
        ptx_parent1->vout[0].nValue + ptx_parent2_v1->vout[0].nValue + ptx_parent3->vout[0].nValue - mixed_child_fee,
        mixed_child_spk);
    ...
    // child should be accepted
    {
        ...
        const CAmount parent3_fee = GetFeePaid(*m_coinbase_txns[3], 0, *ptx_parent3);
        const CFeeRate expected_feerate(
            parent3_fee + mixed_child_fee,
            GetVirtualTransactionSize(*ptx_parent3) + GetVirtualTransactionSize(*ptx_mixed_child));
