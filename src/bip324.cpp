...
{
    InitializeInternal(their_pubkey, initiator, MakeUCharSpan(Params().MessageStart()), self_decrypt);
}

void BIP324Cipher::InitializeForTest(const EllSwiftPubKey& their_pubkey, bool initiator, Span<const unsigned char> message_start, bool self_decrypt) noexcept
{
    InitializeInternal(their_pubkey, initiator, message_start, self_decrypt);
}

void BIP324Cipher::InitializeInternal(const EllSwiftPubKey& their_pubkey, bool initiator, Span<const unsigned char> message_start, bool self_decrypt) noexcept
{
