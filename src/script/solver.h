...
enum class TxoutType {
    ...
    WITNESS_V0_KEYHASH,
    WITNESS_V1_TAPROOT,
    EUTXO_COMMITMENT, //!< Quantum Quasar EUTXO datum/validator commitment
    WITNESS_UNKNOWN, //!< Only for Witness versions not already defined above
    RGB_COMMITMENT,  //!< Quantum Quasar Layer 2 RGB state transition anchor
};

/** Get the name of a TxoutType as a string */
std::string GetTxnOutputType(TxoutType t);

/** Create an RGB Commitment script */
CScript CreateRGBCommitment(const uint256& state_hash);
/** Create an EUTXO datum/validator commitment script */
