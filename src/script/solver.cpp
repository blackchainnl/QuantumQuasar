...
std::string GetTxnOutputType(TxoutType t)
{
    ...
    case TxoutType::EUTXO_COMMITMENT: return "eutxo_commitment";
    case TxoutType::WITNESS_UNKNOWN: return "witness_unknown";
    case TxoutType::RGB_COMMITMENT: return "rgb_commitment";
...
TxoutType Solver(const CScript& scriptPubKey, std::vector<std::vector<unsigned char>>& vSolutionsRet)
{
    ...
    if (scriptPubKey.IsWitnessProgram(witnessversion, witnessprogram)) {
        ...
        if (IsEUTXOWitnessProgram(witnessversion, witnessprogram)) {
            ...
            vSolutionsRet.push_back(std::move(witnessprogram));
            return TxoutType::EUTXO_COMMITMENT;
        }
        if (witnessversion != 0) {
    ...
    if (scriptPubKey.size() >= 1 && scriptPubKey[0] == OP_RETURN && scriptPubKey.IsPushOnly(scriptPubKey.begin()+1)) {
        std::vector<unsigned char> data;
        opcodetype opcode;
        CScript::const_iterator it = scriptPubKey.begin() + 1;
        if (scriptPubKey.GetOp(it, opcode, data) && it == scriptPubKey.end() &&
            data.size() == 4 + uint256::size() &&
            data[0] == 'R' && data[1] == 'G' && data[2] == 'B' && data[3] == '1') {
            vSolutionsRet.push_back(std::move(data));
            return TxoutType::RGB_COMMITMENT;
        }
...
CScript CreateRGBCommitment(const uint256& state_hash)
{
    CScript script;
    script << OP_RETURN;
    std::vector<unsigned char> data{'R', 'G', 'B', '1'};
    data.resize(data.size() + uint256::size());
    std::reverse_copy(state_hash.begin(), state_hash.end(), data.end() - uint256::size());
    script << data;
    return script;
}

CScript CreateEUTXOCommitment(const std::vector<unsigned char>& datum, const CScript& validator_script)
{
    return GetScriptForEUTXO(datum, validator_script);
}
