...
void BaseIndex::BlockDisconnected(const std::shared_ptr<const CBlock>& block, const CBlockIndex* pindex)
{
    ...
    if (best_block_index->GetAncestor(pindex->nHeight) != pindex) {
        LogPrintf("%s: WARNING: Disconnected block %s is not on known best "
                  "chain (tip=%s); not updating index\n",
                  __func__, pindex->GetBlockHash().ToString(),
                  best_block_index->GetBlockHash().ToString());
        return;
    }
    ...
    if (!pindex->pprev) {
        LogPrintf("%s: WARNING: Ignoring disconnect for genesis block %s\n",
                  __func__, pindex->GetBlockHash().ToString());
        return;
    }

    if (!Rewind(best_block_index, pindex->pprev)) {
        FatalErrorf("%s: Failed to rewind index %s after block disconnect",
                   __func__, GetName());
    }
}
