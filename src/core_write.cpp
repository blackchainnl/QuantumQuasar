...
void ScriptToUniv(const CScript& script, UniValue& out, bool include_hex, bool include_address, const SigningProvider* provider)
{
    ...
    if (type == TxoutType::RGB_COMMITMENT && solns.size() == 1 && solns[0].size() == 4 + uint256::size()) {
        out.pushKV("rgb_magic", std::string(solns[0].begin(), solns[0].begin() + 4));
        out.pushKV("rgb_state_hash", HexStr(Span<const unsigned char>(solns[0].data() + 4, uint256::size())));
    }
    if (type == TxoutType::EUTXO_COMMITMENT && solns.size() == 2) {
        out.pushKV("witness_version", static_cast<int>(solns[0][0]));
        out.pushKV("witness_program", HexStr(solns[1]));
    }
