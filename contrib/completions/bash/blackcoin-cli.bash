# bash programmable completion for blackcoin-cli(1)
...
_blackcoin_cli() {
    ...
    # save and use original argument to invoke blackcoin-cli for -help, help and RPC
    # as blackcoin-cli might not be in $PATH
    blackcoin_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($blackcoin_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_blackcoin_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
...
complete -F _blackcoin_cli blackcoin-cli
