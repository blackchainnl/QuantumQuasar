...
class BlockDataCopier:
    ...
    def run(self):
        while self.blkCountOut < len(self.blkindex):
            ...
            inLen = su[0] - self.settings['block_header_size'] # length without header
            blk_hdr = self.inF.read(self.settings['block_header_size'])
            inExtent = BlockExtent(self.inFn, self.inF.tell(), inhdr, blk_hdr, inLen)

            self.hash_str = calc_hash_str(blk_hdr[:self.settings['hash_header_size']])
...
if __name__ == '__main__':
    ...
    if 'block_header_size' not in settings:
        settings['block_header_size'] = 80
    if 'hash_header_size' not in settings:
        settings['hash_header_size'] = 80
    ...
    settings['block_header_size'] = int(settings['block_header_size'])
    ...
    if not settings['genesis'] in blkmap and str(settings.get('min_height', 0)) == '0':
