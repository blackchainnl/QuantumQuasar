...
source_name_overrides = {
    'bench_bitcoin': ['bench_bitcoin', 'bench_bench_blackcoin'],
    'libbitcoin_crypto': ['libbitcoin_crypto', 'crypto_libbitcoin_crypto_base_la'],
}
    ...
    with open(makefile, 'r', encoding='utf-8') as file:
        ...
        for line in file.read().splitlines():
            if current_lib:
                ...
                continue
            for lib in libs:
                source_names = source_name_overrides.get(lib, [lib.replace('-', '_')])
                if any(re.search(source_name + '.*_SOURCES \\= \\\\', line) for source_name in source_names):
                    current_lib = lib
...
def parse_config_into_btc_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Blackcoin {version}\""
...
def main():
    parser = argparse.ArgumentParser(description='Blackcoin msbuild configuration initialiser.')
