...
class P2PEvict(BitcoinTestFramework):
    def set_test_params(self):
        self.num_nodes = 1
        # Blackcoin reserves 16 full-relay outbound, 2 block-relay-only,
        # and 1 feeler slot. maxconnections=40 leaves 21 inbound slots. 20
        # inbound peers are protected from eviction:
    ...
    def run_test(self):
        ...
        for _ in range(4):
            ...
            # Blackcoin regtest uses a 10,000 BLK block subsidy. Send the
            # full block body directly so this eviction test exercises the
            # "peer delivered a block" protection, not direct-fetch timing.
            block = create_block(tip, create_coinbase(node.getblockcount() + 1, nValue=REGTEST_POW_SUBSIDY), best_block_time + 1)
            block.solve()
            block_peer.send_blocks_and_test([block], node, success=True, force_send=True)
