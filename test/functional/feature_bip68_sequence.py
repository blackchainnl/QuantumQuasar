...
class BIP68Test(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # This subtest intentionally leaves an accepted spend chain in the
        # mempool. Bitcoin's fixture can let that leak into later mining, but
        # this fork also enforces transaction nTime input ordering at block
        # validation, so keep the acceptance-only fixture isolated.
    ...
    def test_disable_flag(self):
        ...
        parent = self.wallet.send_self_transfer(from_node=self.nodes[0])
        utxo = parent["new_utxo"]

        tx1 = CTransaction()
        tx1.nTime = parent["tx"].nTime + 1
    ...
    def bump_mocktime_after_tip(self, seconds=600):
        tip_time = self.nodes[0].getblockheader(self.nodes[0].getbestblockhash())["time"]
        current_mocktime = self.nodes[0].mocktime or 0
        next_time = max(tip_time, current_mocktime, int(time.time())) + seconds
        self.nodes[0].setmocktime(next_time)
        return next_time

    def mine_empty_blocks(self, count, cur_time):
        for _ in range(count):
            tip_time = self.nodes[0].getblockheader(self.nodes[0].getbestblockhash())["time"]
            current_mocktime = self.nodes[0].mocktime or 0
            next_time = max(cur_time + 600, tip_time + 600, current_mocktime + 600, int(time.time()) + 600)
            self.nodes[0].setmocktime(next_time)
            tmpl = self.nodes[0].getblocktemplate(NORMAL_GBT_REQUEST_PARAMS)
            block = create_block(tmpl=tmpl, ntime=next_time)
            block.solve()
    ...
    def test_sequence_lock_unconfirmed_inputs(self):
        ...
        # Now mine some blocks, but make sure tx2 doesn't get mined. Bitcoin's
        # inherited fixture used prioritisetransaction to lower the effective
        # feerate to 0; this fork may not expose that RPC, so mine explicit
        # empty blocks in that case.
        has_prioritise = "prioritisetransaction" in self.nodes[0].help()
        if has_prioritise:
        ...
        if has_prioritise:
            ...
            cur_time = self.mine_empty_blocks(10, cur_time)
        ...
        # tx3 to be removed.
        cur_time = max(cur_time, self.nodes[0].getblockheader(self.nodes[0].getbestblockhash())["time"] + 600)
        for i in range(2):
    ...
    # Test that version 2 transactions are standard.
    def test_version2_relay(self):
        mini_wallet = MiniWallet(self.nodes[0])
        mini_wallet.rescan_utxos()
        tx = mini_wallet.create_self_transfer()["tx"]
        tx.nVersion = 2
        mini_wallet.sendrawtransaction(from_node=self.nodes[0], tx_hex=tx.serialize().hex())
