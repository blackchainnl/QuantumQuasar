...
MIN_RELAY_FEE = Decimal('0.00100000')
...
class MempoolLimitTest(BitcoinTestFramework):
    ...
    def create_big_transaction_or_reject(self, miniwallet, node, fee, txouts):
        tx = miniwallet.create_self_transfer(fee=fee)["tx"]
        tx.vout.extend(txouts)
        tx_hex = tx.serialize().hex()
        res = node.testmempoolaccept(rawtxs=[tx_hex], maxfeerate=0)[0]
        if not res.get("allowed", False):
            return res
        assert_equal(res["fees"]["base"], fee)
        return {"allowed": True, "txid": node.sendrawtransaction(hexstring=tx_hex, maxfeerate=0)}
    ...
    def fill_mempool(self):
        ...
        self.log.info("Fill the mempool until size-limit eviction is triggered")
        ...
        for batch_of_txid in range(num_of_batches):
            ...
            for _ in range(tx_batch_size):
                res = self.create_big_transaction_or_reject(miniwallet, node, fee, txouts)
                if res.get("allowed", False):
                    continue
                rejected = res
                self.log.debug(f"Stopped filling mempool after expected rejection: {rejected}")
                break
            if rejected is not None:
                break

        if rejected is not None:
            assert any(reason in rejected.get("reject-reason", "") for reason in ["mempool full", "mempool min fee not met"])
        ...
        self.log.debug("Check that mempoolminfee did not drop below minrelaytxfee")
        assert_equal(node.getmempoolinfo()['minrelaytxfee'], MIN_RELAY_FEE)
        assert_equal(node.getmempoolinfo()['mempoolminfee'], MIN_RELAY_FEE)
    ...
    def test_rbf_carveout_disallowed(self):
        ...
        # This fork rejects the package RBF conflict before reaching the inherited
        # descendant-limit carveout error.
        assert_raises_rpc_error(-26, "txn-mempool-conflict", node.submitpackage, [tx_B["hex"], tx_C["hex"]])
        resulting_mempool_txids = node.getrawmempool()
        assert tx_A["txid"] in resulting_mempool_txids
        assert tx_B["txid"] not in resulting_mempool_txids
        assert tx_C["txid"] not in resulting_mempool_txids
    ...
    def test_mid_package_eviction(self):
        ...
        # looked up, but disappears later. Keep it above this fork's fixed min relay so package
        # processing reaches the intended mid-package eviction path.
        ...
        assert node.testmempoolaccept(rawtxs=[cpfp_parent["hex"]], maxfeerate=0)[0]["allowed"]
        ...
        # This fork evicts with sizelimit removals without logging a rolling-minimum-fee bump.
    ...
    def test_mid_package_replacement(self):
        ...
        # This parent spends the above mempool transaction that exists when its inputs are first
        # looked up, but disappears later. In this fork the rolling mempool minimum stays at
        # minrelaytxfee, so keep it above min relay to reach the replacement/missing-input path.
        cpfp_parent = self.wallet.create_self_transfer(
        ...
        # This fork rejects the later replacement as a mempool conflict before
        # accepting it and invalidating the earlier dependent package member.
        ...
        resulting_mempool_txids = node.getrawmempool()
        # The original transaction remains. The first parent is accepted as a
        # valid descendant before the later conflicting replacement is rejected.
        assert replacement_tx["txid"] not in resulting_mempool_txids
        assert replaced_tx["txid"] in resulting_mempool_txids
        assert cpfp_parent["txid"] in resulting_mempool_txids
        assert child["txid"] not in resulting_mempool_txids
    ...
    def run_test(self):
        ...
        # This fork keeps mempoolminfee at minrelaytxfee during the size-limit
        # eviction path, so test a tx below the fork minimum relay fee instead.
        self.log.info('Create a mempool tx that will not pass minrelaytxfee')
        assert_raises_rpc_error(-26, "bad-txns-fee-not-enough", miniwallet.send_self_transfer, from_node=node, fee_rate=relayfee / 2)

        if "prioritisetransaction" not in node.help():
            self.log.info("Skipping modified-fee CPFP package subcase; this fork build does not expose prioritisetransaction")
        else:
        ...
        # This fork keeps mempoolminfee at minrelaytxfee in this size-limit path,
        # so the parent must be individually above min relay to reach the intended
        # package-size eviction check.
        ...
        assert_greater_than((parent_fee) / (tx_parent_just_below["tx"].get_vsize()), mempoolmin_feerate / 1000)
