...
class MiniWallet:
    ...
    def generate(self, num_blocks, **kwargs):
        ...
        invalid_call = kwargs.pop("invalid_call", False)
        assert not invalid_call
        blocks = self._test_node.generatetodescriptor(num_blocks, self.get_descriptor(), invalid_call=False, **kwargs)
    ...
    def send_to(self, *, from_node, scriptPubKey, amount, fee=DEFAULT_FEE_SATS):
        """
    ...
    def create_self_transfer_multi(
        ...
        *,
        ...
        locktime=0,
        sequence=0,
        fee_per_output=DEFAULT_FEE_SATS,
        target_weight=0,
        ...
        # create tx
        tx = CTransaction()
        tx.nTime = max(tx.nTime, self._test_node.getblock(self._test_node.getbestblockhash())["time"])
        if self._test_node.mocktime is not None:
            tx.nTime = max(tx.nTime, self._test_node.mocktime)
    ...
    def create_self_transfer(self, *,
        ...
        if self._mode in (MiniWalletMode.RAW_OP_TRUE, MiniWalletMode.ADDRESS_OP_TRUE):
            ...
            assert False
        if target_weight:
            vsize = max(vsize, Decimal((target_weight + 3) // 4))
