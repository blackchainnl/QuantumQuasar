...
def chain_subdir_name(chain):
    """Return the datadir subdirectory used by this fork for a chain name."""
    if chain == 'testnet3':
        return 'testnet'
    return chain
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "blackcoin.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> Tuple[dict, pathlib.Path]:
    ...
    if sys.platform == "win32":
        ...
        datadir = temp_dir / "Blackcoin"
    else:
        env = dict(HOME=str(temp_dir))
        if sys.platform == "darwin":
            datadir = temp_dir / "Library/Application Support/Blackcoin"
        else:
            datadir = temp_dir / ".blackcoin"
    return env, datadir
...
def get_auth_cookie(datadir, chain):
    ...
    try:
        with open(os.path.join(datadir, chain_subdir_name(chain), ".cookie"), 'r', encoding="ascii") as f:
            userpass = f.read()
...
def delete_cookie_file(datadir, chain):
    cookie_path = os.path.join(datadir, chain_subdir_name(chain), ".cookie")
