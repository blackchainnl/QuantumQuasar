...
from test_framework.descriptors import descsum_create as _descsum_create, xkeys_to_main_prefix
...
def descsum_create(desc):
    return _descsum_create(xkeys_to_main_prefix(desc))


def bcrt_to_blrt(addr):
    version, program = decode_segwit_address("bcrt", addr)
    assert version is not None
    return encode_segwit_address("blrt", version, program)
...
class ImportDescriptorsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        addresses += [bcrt_to_blrt("bcrt1qrd3n235cj2czsfmsuvqqpr3lu6lg0ju7scl8gn"), bcrt_to_blrt("bcrt1qfqeppuvj0ww98r6qghmdkj70tv8qpchehegrg8")] # wpkh subscripts corresponding to the above addresses
        ...
        addresses = [
            bcrt_to_blrt('bcrt1qtmp74ayg7p24uslctssvjm06q5phz4yrxucgnv'), # m/0'/0'/0
            bcrt_to_blrt('bcrt1q8vprchan07gzagd5e6v9wd7azyucksq2xc76k8'), # m/0'/0'/1
            bcrt_to_blrt('bcrt1qtuqdtha7zmqgcrr26n2rqxztv5y8rafjp9lulu'), # m/0'/0'/2
            bcrt_to_blrt('bcrt1qau64272ymawq26t90md6an0ps99qkrse58m640'), # m/0'/0'/3
            bcrt_to_blrt('bcrt1qsg97266hrh6cpmutqen8s4s962aryy77jp0fg0'), # m/0'/0'/4
        ]
        ...
        self.generatetoaddress(self.nodes[0], 6, w0.getnewaddress(), sync_fun=self.no_op)
        self.sync_blocks()
        ...
        self.test_importdesc({"desc": descsum_create("wsh(multi(2," + xprv1 + "/84h/0h/0h/*," + xprv2 + "/84h/0h/0h/*," + xprv3 + "/84h/0h/0h/*))"),
                            "active": True,
                            "range": 1000,
                            "next_index": 0,
                            "timestamp": "now"},
                            success=True,
                            wallet=wmulti_priv)
        self.test_importdesc({"desc": descsum_create("wsh(multi(2," + xprv1 + "/84h/1h/0h/*," + xprv2 + "/84h/1h/0h/*," + xprv3 + "/84h/1h/0h/*))"),
                            "active": True,
                            "internal" : True,
                            "range": 1000,
                            "next_index": 0,
        ...
        assert_equal(addr, bcrt_to_blrt('bcrt1qdt0qy5p7dzhxzmegnn4ulzhard33s2809arjqgjndx87rv5vd0fq2czhy8')) # Derived at m/84'/0'/0'/0
        change_addr = wmulti_priv.getrawchangeaddress('bech32') # uses change 0
        assert_equal(change_addr, bcrt_to_blrt('bcrt1qt9uhe3a9hnq7vajl7a094z4s3crm9ttf8zw3f5v9gr2nyd7e3lnsy44n8e')) # Derived at m/84'/1'/0'/0
        ...
        self.test_importdesc({"desc": descsum_create("wsh(multi(2,[7b2d0242/84h/0h/0h]" + acc_xpub1 + "/*,[59b09cd6/84h/0h/0h]" + acc_xpub2 + "/*,[e81a0532/84h/0h/0h]" + acc_xpub3 +"/*))"),
                            "active": True,
                            "range": 1000,
                            "next_index": 0,
                            "timestamp": "now"},
                            success=True,
                            wallet=wmulti_pub)
        self.test_importdesc({"desc": descsum_create("wsh(multi(2,[7b2d0242/84h/1h/0h]" + chg_xpub1 + "/*,[59b09cd6/84h/1h/0h]" + chg_xpub2 + "/*,[e81a0532/84h/1h/0h]" + chg_xpub3 + "/*))"),
                            "active": True,
                            "internal" : True,
                            "range": 1000,
                            "next_index": 0,
        ...
        assert_equal(addr, bcrt_to_blrt('bcrt1qp8s25ckjl7gr6x2q3dx3tn2pytwp05upkjztk6ey857tt50r5aeqn6mvr9')) # Derived at m/84'/0'/0'/1
        change_addr = wmulti_pub.getrawchangeaddress('bech32') # uses change 2
        assert_equal(change_addr, bcrt_to_blrt('bcrt1qp6j3jw8yetefte7kw6v5pc89rkgakzy98p6gf7ayslaveaxqyjusnw580c')) # Derived at m/84'/1'/0'/2
        ...
        self.generatetoaddress(self.nodes[0], COINBASE_MATURITY + 1, temp_wallet.getnewaddress(), sync_fun=self.no_op)
        self.generatetoaddress(self.nodes[0], COINBASE_MATURITY + 1, temp_wallet.getnewaddress(), sync_fun=self.no_op)
        self.sync_blocks()
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as thread:
            with self.nodes[0].assert_debug_log(expected_msgs=["Rescan started from block"], timeout=5):
