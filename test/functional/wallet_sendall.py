...
ECONOMIC_DUST_1 = Decimal("0.00019000")
ECONOMIC_DUST_2 = Decimal("0.00020000")
NEGATIVE_EFFECTIVE_FEE_RATE = 10000
...
class SendallTest(BitcoinTestFramework):
    ...
    def sendall_negative_effective_value(self):
        ...
        self.def_wallet.sendtoaddress(dust_wallet.getnewaddress(), ECONOMIC_DUST_1)
        self.def_wallet.sendtoaddress(dust_wallet.getnewaddress(), ECONOMIC_DUST_2)
        self.generate(self.nodes[0], 1)
        ...
        assert_raises_rpc_error(-6, "Total value of UTXO pool too low to pay for transaction",
                dust_wallet.sendall, recipients=[self.remainder_target], fee_rate=NEGATIVE_EFFECTIVE_FEE_RATE)

        dust_wallet.unloadwallet()

    def sendall_with_send_max(self):
        self.log.info("Check that `send_max` leaves UTXOs that are uneconomic at the requested fee rate")
        self.nodes[0].createwallet("sendmaxwallet")
        sendmax_wallet = self.nodes[0].get_wallet_rpc("sendmaxwallet")
        try:
            self.def_wallet.sendtoaddress(sendmax_wallet.getnewaddress(), ECONOMIC_DUST_1)
            self.def_wallet.sendtoaddress(sendmax_wallet.getnewaddress(), ECONOMIC_DUST_2)
            self.def_wallet.sendtoaddress(sendmax_wallet.getnewaddress(), 1)
            self.generate(self.nodes[0], 1)

            sendall_tx_receipt = sendmax_wallet.sendall(recipients=[self.remainder_target], fee_rate=NEGATIVE_EFFECTIVE_FEE_RATE, send_max=True)
            tx_from_wallet = sendmax_wallet.gettransaction(txid = sendall_tx_receipt["txid"], verbose = True)

            assert_equal(len(tx_from_wallet["decoded"]["vin"]), 1)
            self.assert_tx_has_outputs(tx_from_wallet, [{"address": self.remainder_target, "value": Decimal("1") + tx_from_wallet["fee"]}])
            remaining_unspent = sum(utxo["amount"] for utxo in sendmax_wallet.listunspent())
            assert_equal(remaining_unspent, ECONOMIC_DUST_1 + ECONOMIC_DUST_2)
        finally:
            sendmax_wallet.unloadwallet()
    ...
    def sendall_fails_on_high_fee(self):
        self.log.info("Test sendall fails if the transaction fee exceeds -maxtxfee")
        self.add_utxos([21])
        ...
        assert_raises_rpc_error(
                ...
                self.wallet.sendall,
                recipients=[self.remainder_target],
                options={"fee_rate": 10000000})

    @cleanup
    def sendall_fails_on_explicit_low_fee(self):
        self.log.info("Test sendall rejects an explicit fee rate below the relay floor")
        self.add_utxos([1])
        assert_raises_rpc_error(
                -8,
                "Fee rate (99.999 sat/vB) is lower than the minimum fee rate setting (100.000 sat/vB)",
                self.wallet.sendall,
    ...
    def sendall_with_minconf(self):
        ...
        self.wallet.sendall(recipients=[self.remainder_target], options={"fee_rate": 300, "minconf": 6})
        ...
        self.wallet.sendall(recipients=[self.remainder_target], options={"fee_rate": 300, "minconf": 3})
    ...
    def sendall_with_maxconf(self):
        ...
        self.wallet.sendall(recipients=[self.remainder_target], options={"fee_rate": 300, "maxconf":4})
        assert_equal(len(self.wallet.listunspent()), 1)
    ...
    def sendall_fails_with_transaction_too_large(self):
        ...
        # create many inputs
        outputs = {self.wallet.getnewaddress(): 0.00025000 for _ in range(1600)}
    ...
    def run_test(self):
        ...
        # Sendall fails when providing a fee that exceeds -maxtxfee
        self.sendall_fails_on_high_fee()

        # Sendall rejects fees below this fork's minimum fee floor
        self.sendall_fails_on_explicit_low_fee()
