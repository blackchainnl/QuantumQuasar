...
from test_framework.blocktools import create_block, create_coinbase, get_quantum_quasar_min_tx_fee
...
class InvalidTxRequestTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        def set_mocktime_for_next_manual_block():
            """Return a valid manual block time and align tx acceptance to it.

            This fork records block-created coin time and rejects descendants
            whose spend time is earlier than their input coin time. Aligning
            mocktime with the hand-built parent block keeps post-connect mempool
            consistency checks faithful while still satisfying block MTP.
            """
            tip_header = node.getblock(node.getbestblockhash())
            next_time = max(node.mocktime or 0, tip_header["time"] + 1, tip_header["mediantime"] + 1)
        ...
        block1 = block
        node.p2ps[0].send_blocks_and_test([block], node, success=True, force_send=True)
        ...
        # * p2ps[0] will send valid orphan txs
        # * p2ps[1] will send an invalid orphan tx (and is later disconnected for that)
        self.reconnect_p2p(num_connections=2)
        block_A_time = set_mocktime_for_next_manual_block()
        ...
        tx_withhold.vout = [CTxOut(nValue=25 * COIN - valid_fee, scriptPubKey=SCRIPT_PUB_KEY_OP_TRUE)] * 2
        ...
        tx_orphan_2_valid.vout.append(CTxOut(nValue=8 * COIN - valid_fee, scriptPubKey=SCRIPT_PUB_KEY_OP_TRUE))
        ...
        node.p2ps[0].send_txs_and_test([tx_orphan_1, tx_orphan_2_valid], node, success=False)
        ...
        tx_orphan_include_by_block_A.vout.append(CTxOut(nValue=12 * COIN, scriptPubKey=SCRIPT_PUB_KEY_OP_TRUE))
        tx_orphan_include_by_block_A.vout[0].nValue -= get_quantum_quasar_min_tx_fee(tx_orphan_include_by_block_A)
        ...
        block_A = create_block(tip, create_coinbase(height), block_A_time)
        ...
        with node.assert_debug_log(["Erased 1 orphan tx included or conflicted by block"]):
            node.p2ps[0].send_blocks_and_test([block_A], node, success=True, force_send=True)
        ...
        block_B_time = set_mocktime_for_next_manual_block()
        ...
        tip = int(best_block_hash, 16)
        height = node.getblockcount() + 1
        block_B = create_block(tip, create_coinbase(height), block_B_time)
        ...
        with node.assert_debug_log(["Erased 1 orphan tx included or conflicted by block"]):
            node.p2ps[0].send_blocks_and_test([block_B], node, success=True, force_send=True)
