...
class InvalidBlockRequestTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        block1 = block
        peer.send_blocks_and_test([block1], node, success=True, force_send=True)
        ...
        tx1 = create_tx_with_script(block1.vtx[0], 0, script_sig=bytes([OP_TRUE]), amount=50 * COIN - MIN_RELAY_FEE)
        tx2 = create_tx_with_script(tx1, 0, script_sig=bytes([OP_TRUE]), amount=50 * COIN - 2 * MIN_RELAY_FEE)
        ...
        peer.send_blocks_and_test([block2], node, success=False, reject_reason='bad-txns-duplicate', force_send=True)
        ...
        block2_dup.solve()
        peer.send_blocks_and_test([block2_dup], node, success=False, reject_reason='bad-txns-inputs-duplicate', force_send=True)

        self.log.info("Test very broken block.")

        block3 = create_block(tip, create_coinbase(height, nValue=10001), block_time)
        block_time += 1
        block3.solve()

        peer.send_blocks_and_test([block3], node, success=False, reject_reason='bad-cb-amount', force_send=True)
        ...
        peer.send_blocks_and_test([block2_orig], node, success=True, timeout=5, force_send=True)
        ...
        tx3 = create_tx_with_script(tx2, 0, script_sig=bytes([OP_TRUE]), amount=50 * COIN - 3 * MIN_RELAY_FEE)
        tx3.vin.append(tx3.vin[0])  # Duplicates input
        tx3.rehash()
        block4 = create_block(tip, create_coinbase(height), block_time, txlist=[tx3])
        block4.solve()
        self.log.info("Test inflation by duplicating input")
        peer.send_blocks_and_test([block4], node, success=False,  reject_reason='bad-txns-inputs-duplicate', force_send=True)
        ...
        peer.send_blocks_and_test([block], node, success=True, force_send=True)
