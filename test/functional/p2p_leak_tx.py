...
class P2PLeakTxTest(BitcoinTestFramework):
    def set_test_params(self):
        self.num_nodes = 1
        # This test asserts transaction-upload privacy, not trickle timing.
        # Whitelist localhost so tx announcements are deterministic under the
        # parallel functional-suite runner.
    ...
    def test_notfound_on_replaced_tx(self):
        ...
        tx_b = tx_a["tx"]
        tx_b.vout[0].nValue -= 100_000
        tx_b.rehash()
        try:
            self.gen_node.sendrawtransaction(tx_b.serialize().hex())
        except JSONRPCException as exc:
            if exc.error["code"] == -26 and "txn-mempool-conflict" in exc.error["message"]:
                self.log.info("Skipping replacement notfound subtest; this fork does not replace the inherited tx fixture")
                return
            raise
