...
class WalletMultisigDescriptorPSBTTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        for _ in range(10):  # we check that the first 10 generated addresses are the same for all participant's multisigs
            receive_addresses = [multisig.getnewaddress(address_type="bech32") for multisig in participants["multisigs"]]
            all(address == receive_addresses[0] for address in receive_addresses)
            change_addresses = [multisig.getrawchangeaddress(address_type="bech32") for multisig in participants["multisigs"]]
        ...
        deposit_amount = 6.15
        multisig_receiving_address = participants["multisigs"][0].getnewaddress(address_type="bech32")
        ...
        psbt = participants["multisigs"][0].walletcreatefundedpsbt(
            inputs=[],
            outputs={to: value},
            options={"change_type": "bech32", "feeRate": 0.001},
        )
