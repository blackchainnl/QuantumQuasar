...
class WalletGroupTest(BitcoinTestFramework):
    ...
    def set_test_params(self):
        ...
        self.extra_args = [
            ...
            [],
            ["-avoidpartialspends"],
            ["-maxapsfee=0.00020000"],
            ["-maxapsfee=0.00030000"],
        ]

        for args in self.extra_args:
            args.append("-whitelist=noban@127.0.0.1")   # whitelist peers to speed up tx relay / mempool sync
            args.append(f"-paytxfee={100 * 1e3 / 1e8}")  # apply the fork min relay feerate of 100 sats/vB
    ...
    def run_test(self):
        ...
        assert_approx(v[1], vexp=0.3, vspan=0.0003)

        txid2 = self.nodes[2].sendtoaddress(self.nodes[0].getnewaddress(), 0.2)
        self.relay_wallet_tx(2, txid2)
        ...
        v.sort()
        assert_approx(v[0], vexp=0.2, vspan=0.0001)
        assert_approx(v[1], vexp=1.3, vspan=0.0005)
        ...
        # - C1 0.5      - F  ~1.3
        # - D ~0.3
        assert_approx(self.nodes[1].getbalance(), vexp=4.3, vspan=0.0005)
        assert_approx(self.nodes[2].getbalance(), vexp=4.3, vspan=0.0005)
        # Sending 1.4 BLK should spend a small number of inputs and preserve
        # change. Descriptor wallet grouping can make the grouped/non-grouped
        # debug branch fee-equivalent here, so the explicit maxapsfee checks
        # below cover the branch controls deterministically.
        txid3 = self.nodes[1].sendtoaddress(self.nodes[0].getnewaddress(), 1.4)
        self.relay_wallet_tx(1, txid3)
        tx3 = self.nodes[1].getrawtransaction(txid3, True)
        assert 2 <= len(tx3["vin"]) <= 4
        ...
        sendall_receipt = self.nodes[2].sendall(recipients=[self.nodes[0].getnewaddress()])
        self.relay_wallet_tx(2, sendall_receipt["txid"])
        self.sync_all()
        self.generate(self.nodes[0], 1)
        self.sync_mocktime_to_tip()
