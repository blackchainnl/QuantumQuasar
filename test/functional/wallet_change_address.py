    ...
    def run_test(self):
        ...
        for i in range(20):
            for n in [1, 2]:
                ...
                # Blackcoin keeps Blackcoin's higher fee/change economics.
                # The inherited 0.2 BLK spend can exhaust the small confirmed
                # funding set and legitimately produce a no-change transaction
                # before 20 keypool indexes are exercised. Use a smaller amount
                # so every iteration still has an economically meaningful change
                # output and the test remains about change-address sequencing.
                txid = self.nodes[n].sendtoaddress(self.nodes[0].getnewaddress(), 0.01)
        ...
        # With this fork's BLK fee/change economics and the test-local
        # -discardfee=1 setting, the inherited 3.1 BLK funding can discard the
        # remaining change when spending 2.9999 BLK. Fund enough value so the
        # configured change_position path is actually exercised.
        self.nodes[0].sendtoaddress(addr1, 4.0)
        self.nodes[0].sendtoaddress(addr1, 0.1)
        self.nodes[0].sendtoaddress(addr2, 4.0)
