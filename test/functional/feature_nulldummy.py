...
NULLDUMMY_ERROR = "Dummy CHECKMULTISIG argument must be zero"
...
class NULLDUMMYTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Test 3: Non-NULLDUMMY base transactions should be rejected in blocks")
        self.block_submit(self.nodes[0], [test2tx], accept=False)

        self.log.info(f"Advancing to segwit activation [{COINBASE_MATURITY + 5}]")
        self.block_submit(self.nodes[0], [], accept=True)

        self.log.info("Test 4: Non-NULLDUMMY base multisig transaction is invalid after activation")
        test4tx = self.create_transaction(txid=txid2, input_details=ms_unlock_details,
    ...
    def block_submit(self, node, txs, *, with_witness=False, accept):
        ...
        if accept:
            ...
            assert NULLDUMMY_ERROR in result
        if accept:
