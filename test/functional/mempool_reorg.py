...
class MempoolCoinbaseTest(BitcoinTestFramework):
    ...
    def test_reorg_relay(self):
        ...
        # Connect a peer to node1. Normal tx relay timing can race under the
        # parallel runner, so the unannounced-too-recent subcheck below is only
        # asserted when this peer actually remains unannounced.
        request_peer = self.nodes[1].add_p2p_connection(P2PTxInvStore())
        ...
        early_relay = bool(request_peer.get_invs())
        if early_relay:
            self.log.info("Peer received early tx invs before the reorg; skipping unannounced-too-recent subcheck")
            with p2p_lock:
                request_peer.tx_invs_received.clear()
        ...
        post_reorg_relay = bool(request_peer.get_invs())
        if not early_relay and not post_reorg_relay:
            # It's too early to request these two transactions if they have not
            ...
            request_peer.send_and_ping(requests_too_recent)
            for _ in range(len(requests_too_recent.inv)):
                request_peer.sync_with_ping()
            with p2p_lock:
                assert "tx" not in request_peer.last_message
                assert "notfound" in request_peer.last_message
        else:
            self.log.info("Peer received tx invs around the reorg; request path is no longer unannounced")
            with p2p_lock:
                request_peer.tx_invs_received.clear()
        ...
        request_peer.send_and_ping(request_disconnected_tx)

        # The node will answer requests for the tx from the recently-disconnected
        # block immediately, regardless of whether normal tx relay won the timing
        # race above.
        with p2p_lock:
            assert_equal(request_peer.last_message["tx"].tx.getwtxid(), tx_disconnected["tx"].getwtxid())

        if not early_relay and not post_reorg_relay:
            self.nodes[1].setmocktime(int(time.time()) + 300)
            request_peer.sync_with_ping()
            # The two previously unannounced mempool transactions are now
            # announced. The disconnected-block tx was requested explicitly
            # above, so it may already be marked known by this peer and is not
            # required here.
            expected_delayed_invs = {int(tx["tx"].getwtxid(), 16) for tx in [tx_before_reorg, tx_child]}
            request_peer.wait_until(lambda: expected_delayed_invs.issubset(set(request_peer.tx_invs_received.keys())), timeout=10)
            for _ in range(len(request_peer.get_invs())):
                # make sure all tx requests have been responded to
                request_peer.sync_with_ping()
            last_tx_received = request_peer.last_message["tx"]
            ...
            request_peer.send_and_ping(request_after_reorg)
            with p2p_lock:
                assert_equal(request_peer.last_message["tx"], last_tx_received)
        else:
            self.log.info("Skipping delayed-announcement subcheck because normal tx relay already announced to this peer")
