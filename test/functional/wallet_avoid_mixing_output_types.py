...
random value payments which sum to 10BTC for each supported output type to Bob,
...
def is_same_type(node, tx):
    ...
    for addr in inputs:
        ...
        if is_bech32m_address(node, addr):
            raise AssertionError("bech32m/Taproot inputs are not expected because this fork does not expose bech32m wallet addresses")
...
class AddressInputTypeGrouping(BitcoinTestFramework):
    ...
    def make_payment(self, A, B, v, addr_type):
        fee_rate = random.randint(100, 200)
    ...
    def run_test(self):
        ...
        for v in generate_payment_values(5, 9):
            ...
            self.generate(B, 1, sync_fun=self.no_op)
            self.sync_blocks()
