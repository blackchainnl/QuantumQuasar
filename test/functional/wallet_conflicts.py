...
class TxConflicts(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        tx_AB_parent_amount = Decimal("20") - RAW_TX_FEE
        tx_AB_parent = self.nodes[0].signrawtransactionwithwallet(self.nodes[0].createrawtransaction(inputs_tx_AB_parent, {self.nodes[0].getnewaddress(): tx_AB_parent_amount}))
        ...
        tx_A_1 = self.nodes[0].signrawtransactionwithwallet(self.nodes[0].createrawtransaction(inputs_tx_A_1, {self.nodes[0].getnewaddress(): Decimal("10") - RAW_TX_FEE}))
        tx_B_1 = self.nodes[0].signrawtransactionwithwallet(self.nodes[0].createrawtransaction(inputs_tx_B_1, {self.nodes[0].getnewaddress(): Decimal("10") - RAW_TX_FEE}))
        ...
        output_c = self.get_utxo_of_value(from_tx_id=txid_AB_parent, search_value=tx_AB_parent_amount)
        inputs_tx_C_child = [({"txid": txid_AB_parent, "vout": output_c})]

        tx_C_child = self.nodes[0].signrawtransactionwithwallet(self.nodes[0].createrawtransaction(inputs_tx_C_child, {self.nodes[0].getnewaddress() : tx_AB_parent_amount - RAW_TX_FEE}))
