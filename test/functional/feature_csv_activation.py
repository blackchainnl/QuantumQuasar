...
class BIP68_112_113Test(BitcoinTestFramework):
    ...
    def set_fixture_tx_time(self, tx):
        # Version 1 Blackcoin transactions serialize nTime. This test builds a
        # synthetic chain in the past, so inherited wall-clock tx timestamps
        # would make otherwise valid blocks fail bad-tx-time.
        tx.nTime = self.last_block_time

    def tip_median_time(self):
        return self.nodes[0].getblockheader(self.nodes[0].getbestblockhash())["mediantime"]

    def create_bip112special(self, input, txversion):
        tx = self.create_self_transfer_from_utxo(input)
        tx.nVersion = txversion
        self.set_fixture_tx_time(tx)
    ...
    def send_blocks(self, blocks, success=True, reject_reason=None):
        ...
        # This is a buried-deployment consensus test, not a P2P block-fetch or
        # unsolicited-block relay test. Submit the synthetic fixture blocks
        # directly so the assertions cover CSV/BIP68/112/113 validation only.
        reject_reason = [reject_reason] if reject_reason else []
        with self.nodes[0].assert_debug_log(expected_msgs=reject_reason):
            for block in blocks:
    ...
    def run_test(self):
        ...
        bip113tx_v1.nLockTime = self.tip_median_time()  # = MTP of prior block (not <) but < time put on current block
        self.set_fixture_tx_time(bip113tx_v1)
        ...
        success_txs = []
        # BIP113 tx, -1 CSV tx and empty stack CSV tx should succeed
        bip113tx_v2.nLockTime = self.tip_median_time()  # = MTP of prior block (not <) but < time put on current block
        self.set_fixture_tx_time(bip113tx_v2)
        ...
        bip113tx_v1.nLockTime = self.tip_median_time() - 1  # < MTP of prior block
        self.set_fixture_tx_time(bip113tx_v1)
        self.miniwallet.sign_tx(bip113tx_v1)
        bip113tx_v2.nLockTime = self.tip_median_time() - 1  # < MTP of prior block
        self.set_fixture_tx_time(bip113tx_v2)
        self.miniwallet.sign_tx(bip113tx_v2)
        for bip113tx in [bip113tx_v1, bip113tx_v2]:
