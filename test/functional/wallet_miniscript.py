...
from test_framework.descriptors import descsum_create, xkey_to_main_prefix
from test_framework.psbt import PSBT, PSBT_IN_SHA256
from test_framework.test_framework import BitcoinTestFramework
from test_framework.util import assert_equal


TPRVS = [xkey_to_main_prefix(k) for k in [
...
TPUBS = [xkey_to_main_prefix(k) for k in [
...
class WalletMiniscriptTest(BitcoinTestFramework):
    ...
    def signing_test(
        ...
        if stack_size is not None:
            ...
            if locktime is not None and height < locktime:
                self.generatetoaddress(self.nodes[0], locktime - height, self.funder.getnewaddress())
    ...
    def run_test(self):
        ...
        # The standard suite keeps wsh Miniscript coverage. TapMiniscript vectors
        # require wallet bech32m/Taproot address support, which this fork does
        # not expose in the inherited wallet-address surface.
        for desc in [d for d in DESCS if not d.startswith("tr(")]:
            self.watchonly_test(desc)

        # Test we can sign for any Miniscript.
        for desc in [d for d in DESCS_PRIV if not d["desc"].startswith("tr(")]:
            self.signing_test(
                desc["desc"],
                desc["sequence"],
                desc["locktime"],
        ...
        self.log.info("Skipping TapMiniscript wallet vectors; bech32m/Taproot wallet addresses are not exposed by this fork.")
