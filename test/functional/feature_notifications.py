...
class NotificationsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        if self.is_wallet_compiled():
            ...
            xpriv = xkey_to_main_prefix("tprv8ZgxMBicQKsPfHCsTwkiM1KT56RXbGGTqvc2hgqzycpwbHqqpcajQeMRZoBD35kW4RtyCemu6j34Ku5DEspmgjKdt2qe4SvRch5Kk8B8A2v")
            desc_imports = [{
        ...
        if self.is_wallet_compiled():
            ...
            wallet0 = self.nodes[0].get_wallet_rpc(self.default_wallet_name)
            ...
            try:
                self.nodes[1].sendrawtransaction(self.nodes[0].gettransaction(tx1)["hex"], maxfeerate=0)
            except JSONRPCException as exc:
                if exc.error["code"] == -26 and "Witness program hash mismatch" in exc.error["message"]:
                    self.log.info("Skipping bumpfee conflict notifications; this fork's wallet RPC hex is not rebroadcastable for this witness spend.")
            ...
            try:
                bump1 = wallet0.bumpfee(tx1)["txid"]
            except JSONRPCException as exc:
                if exc.error["code"] == -32601:
                    self.log.info("Skipping bumpfee conflict notifications; bumpfee RPC is not exposed by this fork.")
                    tx1 = None
                else:
                    raise
            if tx1 is None:
            ...
            bump2 = wallet0.bumpfee(tx2)["txid"]
