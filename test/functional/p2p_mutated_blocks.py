...
# Copyright (c) Blackcoin Developers
...
class MutatedBlocksTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        block_height = COINBASE_MATURITY + 1

        # The next block should be the only one in flight from peer 0
        peer_info_prior_to_attack = self.nodes[0].getpeerinfo()
        assert_equal(peer_info_prior_to_attack[0]['id'], 0)
        assert_equal([block_height], peer_info_prior_to_attack[0]["inflight"])
        ...
        assert_equal([block_height], peer_info_after_attack[0]["inflight"])
