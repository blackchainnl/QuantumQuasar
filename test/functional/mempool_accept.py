...
LEGACY_MAX_BLOCK_WEIGHT = 4_000_000
CONSENSUS_MAX_MONEY = 20_000_000_000 * COIN
STANDARD_TEST_FEE = Decimal('0.00020000')
...
class MempoolAcceptanceTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        final_utxo = self.wallet.get_utxo()
        ...
        self.log.info('A transaction that would replace a mempool transaction is rejected')
        ...
        txin = tx.vin[0]
        tx.vin = [
            CTxIn(COutPoint(txin.prevout.hash, n), txin.scriptSig, txin.nSequence)
            for n in range(math.ceil(LEGACY_MAX_BLOCK_WEIGHT // 4 / len(txin.serialize())))
        ]
        self.check_mempool_result(
        ...
        tx = tx_from_hex(raw_tx_reference)
        tx.vout[0].nValue = CONSENSUS_MAX_MONEY + 1
        self.check_mempool_result(
        ...
        tx = tx_from_hex(raw_tx_reference)
        tx.vout = [tx.vout[0]] * 2
        tx.vout[0].nValue = CONSENSUS_MAX_MONEY
        self.check_mempool_result(
        ...
        tx = tx_from_hex(raw_tx_reference)
        tx.nVersion = 5  # A version currently non-standard
        self.check_mempool_result(
        ...
        seed_tx = self.wallet.send_to(
            ...
            amount=COIN,
            fee=int(STANDARD_TEST_FEE * COIN),
        )
        self.generate(node, 1)
        ...
        tx.vout[0] = CTxOut(COIN - int(STANDARD_TEST_FEE * COIN), DUMMY_MIN_OP_RETURN_SCRIPT)
        assert_equal(len(tx.serialize_without_witness()), MIN_STANDARD_TX_NONWITNESS_SIZE)
        self.check_mempool_result(
            result_expected=[{'txid': tx.rehash(), 'allowed': True, 'vsize': tx.get_vsize(), 'fees': { 'base': STANDARD_TEST_FEE}}],
            rawtxs=[tx.serialize().hex()],
            maxfeerate=0,
        )
