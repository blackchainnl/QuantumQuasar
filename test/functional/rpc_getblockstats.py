...
class GetblockstatsTest(BitcoinTestFramework):
    ...
    def generate_test_data(self, filename):
        # Generate fork-native blocks. The inherited upstream fixture contains
        # serialized Bitcoin regtest blocks/genesis, which are invalid for this
        # fork's chain params and transaction serialization.
        mocktime = 1525107225
        ...
        op_return_tx = wallet.create_self_transfer(fee_rate=0)["tx"]
        op_return_tx.vout[0].nValue -= DEFAULT_FEE_SATS
        op_return_tx.vout.append(CTxOut(0, CScript([OP_RETURN, b'\x21'])))
        wallet.sign_tx(op_return_tx)
        op_return_tx.rehash()
        wallet.sendrawtransaction(from_node=self.nodes[0], tx_hex=op_return_tx.serialize().hex())
        self.sync_all()
        self.generate(wallet, 1)
