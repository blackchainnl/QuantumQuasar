...
class BlockstoreReindexTest(BitcoinTestFramework):
    ...
    def reindex_readonly(self):
        ...
        if undo_immutable:
            self.log.info("Attempt to restart and reindex the node with the old read-only block file")
            try:
                # Faithful to the upstream v30 invariant: reindex reads old
                # block files and must not require rewriting an already-closed
                # block file. The fork-specific bug was that -fastprune no
                # longer created the second block file needed to make blk00000
                # an old/non-current file.
