...
class InitStressTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        def check_waitforblockheight_sigterm_interrupts():
            """Ensure SIGTERM wakes long-poll RPC waiters before shutdown."""
            current_height = node.getblockcount()
            longpoll_rpc = get_rpc_proxy(node.url, 1, timeout=600, coveragedir=node.coverage_dir)

            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(longpoll_rpc.waitforblockheight, current_height + 1, 600000)
                self.wait_until(
                    lambda: any(command["method"] == "waitforblockheight" for command in node.getrpcinfo()["active_commands"]),
                    timeout=10,
                )

                self.log.info("Terminating node while waitforblockheight is active")
                node.process.terminate()
                try:
        ...
        check_clean_start()
        check_waitforblockheight_sigterm_interrupts()
        check_clean_start()
        self.stop_node(0)
