...
from test_framework.p2p import NODE_WITNESS, P2PDataStore, P2P_SERVICES
...
class MiningTest(BitcoinTestFramework):
    ...
    def test_blockmintxfee_parameter(self):
        ...
        # Test the fork's default and arbitrary blockmintxfee rates [sat/kvB].
        # Rates below the Blackcoin relay floor are not useful here because the
        # mempool rejects them before block template filtering can be observed.
        for blockmintxfee_sat_kvb in (DEFAULT_BLOCK_MIN_TX_FEE, 125000, 333333, 2500000):
    ...
    def run_test(self):
        ...
        def submitblock_and_check(block, expected):
            try:
                assert_equal(expected, node.submitblock(hexdata=block.serialize().hex()))
            except JSONRPCException as exc:
                assert_equal(exc.error["code"], -100)
                assert expected in exc.error["message"]

        def assert_submitblock(block, result_str_1, result_str_2=None):
            block.solve()
            result_str_2 = result_str_2 or 'duplicate-invalid'
            submitblock_and_check(block, result_str_1)
            submitblock_and_check(block, result_str_2)
        ...
        submitblock_and_check(bad_block_root, 'bad-txnmrklroot')
        submitblock_and_check(bad_block_root, 'bad-txnmrklroot')
        assert chain_tip(bad_block_root.hash) in node.getchaintips()
        ...
        submitblock_and_check(bad_block_lock, 'bad-txns-nonfinal')
        submitblock_and_check(bad_block_lock, 'duplicate-invalid')
        ...
        peer = node.add_p2p_connection(P2PDataStore(), services=P2P_SERVICES | NODE_WITNESS)
