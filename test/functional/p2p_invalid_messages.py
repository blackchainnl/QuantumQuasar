...
RESOURCE_EXHAUSTION_TARGET_BYTES = 320 * 1024 * 1024
...
class InvalidMessagesTest(BitcoinTestFramework):
    ...
    def test_size(self):
        ...
        conn = self.nodes[0].add_p2p_connection(P2PDataStore())
        with self.nodes[0].assert_debug_log([f'Header error: Size too large (badmsg, {MAX_PROTOCOL_MESSAGE_LENGTH + 1} bytes)']):
    ...
    def test_resource_exhaustion(self):
        ...
        message_count = max(1, min(80, RESOURCE_EXHAUSTION_TARGET_BYTES // MAX_PROTOCOL_MESSAGE_LENGTH))
        self.log.info(f"(a) Send {message_count} messages, each of maximum valid data size")
