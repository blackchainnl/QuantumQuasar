...
class ResendWalletTransactionsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Bitcoin Core uses RBF grinding here to manufacture a child/parent wallet
        # ordering edge case. Blackcoin/Blackcoin intentionally disables mempool
        # replacement, so keep the resend coverage but assert the fork policy instead
        # of depending on replacement-only test setup.
        ...
        bumped_raw = node.createrawtransaction(
            inputs=child_inputs,
            outputs=[{addr: child_output_value - Decimal("0.00010000")}],
        )
        bumped = node.signrawtransactionwithwallet(bumped_raw)
        assert_raises_rpc_error(-26, "txn-mempool-conflict", node.sendrawtransaction, bumped["hex"])

        # With replacement disabled, the wallet order is whatever this run produced.
        # The resend check below proves both linked wallet transactions are returned
        # to the mempool after expiry eviction without relying on Bitcoin's RBF-only
        # ordering grinder.
