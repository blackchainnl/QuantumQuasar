...
class MerkleBlockTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Bitcoin's inherited fixture assumes node0 has no txindex, so a
        # fully-spent tx cannot be resolved without a blockhash. This fork may
        # run txindex by default, in which case the proof is available.
        node0_txindex_synced = self.nodes[0].getindexinfo().get("txindex", {}).get("synced", False)
        if node0_txindex_synced:
            assert_equal(self.nodes[0].verifytxoutproof(self.nodes[0].gettxoutproof([txid_spent])), [txid_spent])
        else:
