...
P2WPKH = 0
P2WSH = 1
TEST_FEE = Decimal("0.001")
TEST_FEE_SATS = int(TEST_FEE * COIN)
...
def amount_minus_fee(value, fee=TEST_FEE):
...
class SegWitTest(BitcoinTestFramework):
    ...
    def bump_mocktime_after_tip(self):
        next_time = max(node.getblockheader(node.getbestblockhash())["time"] for node in self.nodes) + 1
        for node in self.nodes:
            node.setmocktime(next_time)

    def sync_and_bump_mocktime(self):
        self.sync_blocks()
        self.bump_mocktime_after_tip()

    def success_mine(self, node, txid, sign, redeem_script=""):
        self.bump_mocktime_after_tip()
        send_to_witness(1, node, getutxo(txid), self.pubkey[0], False, amount_minus_fee(node.gettxout(txid, 0)["value"], 2 * TEST_FEE), sign, redeem_script)
        block = self.generate(node, 1)
        assert_equal(len(node.getblock(block[0])["tx"]), 2)
        self.sync_and_bump_mocktime()

    def fail_accept(self, node, error_msg, txid, sign, redeem_script=""):
        self.bump_mocktime_after_tip()
        assert_raises_rpc_error(-26, error_msg, send_to_witness, use_p2wsh=1, node=node, utxo=getutxo(txid), pubkey=self.pubkey[0], encode_p2sh=False, amount=amount_minus_fee(node.gettxout(txid, 0)["value"], 2 * TEST_FEE), sign=sign, insert_redeem_script=redeem_script)
    ...
    def run_test(self):
        ...
        for _ in range(5):
            for n in range(3):
                for v in range(2):
                    ...
                    wit_ids[n][v].append(send_to_witness(v, self.nodes[0], utxo, self.pubkey[n], False, amount_minus_fee(utxo["amount"])))
                    utxo = find_spendable_utxo(self.nodes[0], 50)
                    p2sh_ids[n][v].append(send_to_witness(v, self.nodes[0], utxo, self.pubkey[n], True, amount_minus_fee(utxo["amount"])))
        ...
        self.fail_accept(self.nodes[2], "Operation not valid with the current stack size", p2sh_ids[NODE_2][P2WPKH][1], sign=False)
        self.fail_accept(self.nodes[2], "Operation not valid with the current stack size", p2sh_ids[NODE_2][P2WSH][1], sign=False)
        ...
        send_to_witness(1, self.nodes[2], getutxo(wit_ids[NODE_2][P2WPKH][0]), self.pubkey[0], encode_p2sh=False, amount=amount_minus_fee(self.nodes[2].gettxout(wit_ids[NODE_2][P2WPKH][0], 0)["value"], 2 * TEST_FEE), sign=True)
        send_to_witness(1, self.nodes[2], getutxo(wit_ids[NODE_2][P2WSH][0]), self.pubkey[0], encode_p2sh=False, amount=amount_minus_fee(self.nodes[2].gettxout(wit_ids[NODE_2][P2WSH][0], 0)["value"], 2 * TEST_FEE), sign=True)
        send_to_witness(1, self.nodes[2], getutxo(p2sh_ids[NODE_2][P2WPKH][0]), self.pubkey[0], encode_p2sh=False, amount=amount_minus_fee(self.nodes[2].gettxout(p2sh_ids[NODE_2][P2WPKH][0], 0)["value"], 2 * TEST_FEE), sign=True)
        send_to_witness(1, self.nodes[2], getutxo(p2sh_ids[NODE_2][P2WSH][0]), self.pubkey[0], encode_p2sh=False, amount=amount_minus_fee(self.nodes[2].gettxout(p2sh_ids[NODE_2][P2WSH][0], 0)["value"], 2 * TEST_FEE), sign=True)
        ...
        self.fail_accept(self.nodes[0], "Witness program hash mismatch", wit_ids[NODE_0][P2WPKH][0], sign=False)
        self.fail_accept(self.nodes[0], "Witness program was passed an empty witness", wit_ids[NODE_0][P2WSH][0], sign=False)
        self.fail_accept(self.nodes[0], "Operation not valid with the current stack size", p2sh_ids[NODE_0][P2WPKH][0], sign=False)
        self.fail_accept(self.nodes[0], "Operation not valid with the current stack size", p2sh_ids[NODE_0][P2WSH][0], sign=False)
        # unsigned with redeem script
        self.fail_accept(self.nodes[0], "Witness program hash mismatch", p2sh_ids[NODE_0][P2WPKH][0], sign=False, redeem_script=witness_script(False, self.pubkey[0]))
        self.fail_accept(self.nodes[0], "Witness program was passed an empty witness", p2sh_ids[NODE_0][P2WSH][0], sign=False, redeem_script=witness_script(True, self.pubkey[0]))

        self.log.info("Verify block and transaction serialization rpcs return differing serializations depending on rpc serialization flag")
        assert self.nodes[1].getblock(blockhash, False) != self.nodes[0].getblock(blockhash, False)
        assert self.nodes[2].getblock(blockhash, False) == self.nodes[0].getblock(blockhash, False)

        for tx_id in segwit_tx_list:
            tx_with_witness = self.nodes[1].getrawtransaction(tx_id, False, blockhash)
            tx_without_witness = self.nodes[0].getrawtransaction(tx_id, False, blockhash)
            tx = tx_from_hex(tx_with_witness)
            assert tx_with_witness != tx_without_witness
            assert self.nodes[2].getrawtransaction(tx_id, False, blockhash) == tx_without_witness
            assert self.nodes[2].gettransaction(tx_id)["hex"] == tx_without_witness
            assert tx_without_witness == tx.serialize_without_witness().hex()
            assert tx_with_witness == tx.serialize_with_witness().hex()
        ...
        self.fail_accept(self.nodes[2], 'Witness program hash mismatch', wit_ids[NODE_2][P2WPKH][2], sign=False)
        self.fail_accept(self.nodes[2], 'Witness program was passed an empty witness', wit_ids[NODE_2][P2WSH][2], sign=False)
        self.fail_accept(self.nodes[2], 'Witness program hash mismatch', p2sh_ids[NODE_2][P2WPKH][2], sign=False, redeem_script=witness_script(False, self.pubkey[2]))
        self.fail_accept(self.nodes[2], 'Witness program was passed an empty witness', p2sh_ids[NODE_2][P2WSH][2], sign=False, redeem_script=witness_script(True, self.pubkey[2]))
        ...
        tmpl = self.nodes[0].getblocktemplate({'rules': ['segwit']})
        assert_equal(tmpl['sizelimit'], MAX_BLOCK_WEIGHT)
        ...
        utxo = find_spendable_utxo(self.nodes[0], 50)
        txid1 = send_to_witness(1, self.nodes[0], utxo, self.pubkey[0], False, amount_minus_fee(utxo["amount"], 4 * TEST_FEE))
        ...
        tx.vout.append(CTxOut(tx1.vout[0].nValue - TEST_FEE_SATS, CScript([OP_TRUE, OP_DROP] * 15 + [OP_TRUE])))
        tx2_hex = self.nodes[0].signrawtransactionwithwallet(tx.serialize().hex())['hex']
        txid2 = self.nodes[0].sendrawtransaction(tx2_hex)
        tx = tx_from_hex(tx2_hex)
        tx2_value = tx.vout[0].nValue
        ...
        tx.vout.append(CTxOut(tx2_value - TEST_FEE_SATS, CScript([OP_TRUE, OP_DROP] * 15 + [OP_TRUE])))
        tx.calc_sha256()
        ...
        if not self.options.descriptors:
            ...
            for use_p2wsh in [False, True]:
                ...
                tx = CTransaction()
                tx.vout.append(CTxOut(100000000, CScript(bytes.fromhex(scriptPubKey))))
                transaction = tx.serialize_without_witness().hex()
