...
# Copyright (c) 2016-2021 Blackcoin Developers
...
class FeeFilterTest(BitcoinTestFramework):
    def set_test_params(self):
        self.num_nodes = 2
        # Blackcoin enforces a consensus minimum fee of 100 sat/vB, so
        # the inherited sub-sat/vB vectors are invalid before feefilter relay
        # can be exercised. Keep the test above that floor and vary only the
        # relative feefilter threshold.
        # grant noban permission to all peers to speed up tx relay / mempool sync
        self.extra_args = [[
            "-minrelaytxfee=0.00100000",
    ...
    def test_feefilter(self):
        ...
        self.log.info("Test txs paying 220 sat/vB are received by test connection")
        txids = [miniwallet.send_self_transfer(fee_rate=Decimal('0.00220000'), from_node=node1)['wtxid'] for _ in range(3)]
        conn.wait_for_invs_to_match(txids)
        conn.clear_invs()

        # Set a fee filter of 200 sat/vB on test connection.
        conn.send_and_ping(msg_feefilter(200000))

        self.log.info("Test txs paying 200 sat/vB are received by test connection")
        txids = [miniwallet.send_self_transfer(fee_rate=Decimal('0.00200000'), from_node=node1)['wtxid'] for _ in range(3)]
        conn.wait_for_invs_to_match(txids)
        conn.clear_invs()

        self.log.info("Test txs paying 150 sat/vB are no longer received by test connection")
        ...
        # as well.
        txids = [miniwallet.send_self_transfer(fee_rate=Decimal('0.00300000'), from_node=node0)['wtxid'] for _ in range(1)]
        conn.wait_for_invs_to_match(txids)
        conn.clear_invs()
        ...
        txids = [miniwallet.send_self_transfer(fee_rate=Decimal('0.00300000'), from_node=node1)['wtxid'] for _ in range(3)]
        conn.wait_for_invs_to_match(txids)
        conn.clear_invs()
