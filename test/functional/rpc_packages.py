...
class RPCPackagesTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        for _ in range(3):
            tx_hex = self.wallet.create_self_transfer(fee_rate=DEFAULT_FEE)["hex"]
    ...
    def test_independent(self, coin):
        ...
        tx_bad_sig_hex = node.createrawtransaction([{"txid": coin["txid"], "vout": 0}],
                                           {address : coin["amount"] - DEFAULT_FEE})
    ...
    def test_multiple_parents(self):
        ...
        for num_parents in [2, 10, 24]:
            ...
            child_tx = self.wallet.create_self_transfer_multi(utxos_to_spend=parent_coins, fee_per_output=DEFAULT_FEE_SATS * num_parents)
            for _ in range(10):
                random.shuffle(package_hex)
                testres_multiple = node.testmempoolaccept(rawtxs=package_hex + [child_tx['hex']])
                assert all([testres.get("allowed") for testres in testres_multiple]), testres_multiple
    ...
    def test_rbf(self):
        ...
        # This transaction is signaled as BIP125 replace-by-fee, but this fork
        # disables mempool replacement policy. Keep the inherited package test
        # faithful by asserting conflict rather than changing node policy.
        ...
        assert_equal(testres_rbf_single[0]["allowed"], False)
        assert_equal(testres_rbf_single[0]["reject-reason"], "txn-mempool-conflict")
    ...
    def test_submit_child_with_parents(self, num_parents, partial_submit):
        ...
        child_tx = self.wallet.create_self_transfer_multi(utxos_to_spend=[tx["new_utxo"] for tx in package_txns], fee_per_output=DEFAULT_FEE_SATS * num_parents)
