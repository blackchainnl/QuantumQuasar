...
class BytesPerSigOpTest(BitcoinTestFramework):
    ...
    def create_p2wsh_spending_tx(self, witness_script, output_script):
        ...
        tx.vout = [CTxOut(0, output_script)]
        return tx
    ...
    def test_sigops_limit(self, bytes_per_sigop, num_sigops):
        ...
        res = self.nodes[0].testmempoolaccept(rawtxs=[tx.serialize().hex()], maxfeerate=0)[0]
        ...
        self.nodes[0].sendrawtransaction(hexstring=tx.serialize().hex(), maxfeerate=0, maxburnamount='1.0')
        ...
        parent_vsize = entry_parent['vsize']
        ...
        assert_equal(entry_child['ancestorsize'], sigop_equivalent_vsize + parent_vsize)

        assert_equal(entry_parent['ancestorcount'], 1)
        assert_equal(entry_parent['ancestorsize'], parent_vsize)
        assert_equal(entry_parent['descendantcount'], 2)
        assert_equal(entry_parent['descendantsize'], parent_vsize + sigop_equivalent_vsize)
