...
class UnconfirmedInputTest(BitcoinTestFramework):
    ...
    def generate(self, generator, *args, **kwargs):
        if generator.mocktime is not None:
            generator.bumpmocktime(1)
        return super().generate(generator, *args, **kwargs)
    ...
    def test_target_feerate_unconfirmed_low(self):
        ...
        parent_txid = wallet.sendtoaddress(address=wallet.getnewaddress(), amount=1, fee_rate=self.low_fee_rate)
        parent_tx = wallet.gettransaction(txid=parent_txid, verbose=True)
    ...
    def test_chain_of_unconfirmed_low(self):
        ...
        grandparent_txid = wallet.sendtoaddress(address=wallet.getnewaddress(), amount=1.8, fee_rate=self.low_fee_rate)
        gp_tx = wallet.gettransaction(txid=grandparent_txid, verbose=True)

        self.assert_undershoots_target(gp_tx)

        parent_txid = wallet.sendtoaddress(address=wallet.getnewaddress(), amount=1.5, fee_rate=self.mid_fee_rate)
    ...
    def test_two_low_feerate_unconfirmed_parents(self):
        ...
        parent_one_txid = wallet.sendtoaddress(address=wallet.getnewaddress(), amount=1.5, fee_rate=self.mid_fee_rate)
        p_one_tx = wallet.gettransaction(txid=parent_one_txid, verbose=True)
        self.assert_undershoots_target(p_one_tx)

        parent_two_txid = wallet.sendtoaddress(address=wallet.getnewaddress(), amount=1.5, fee_rate=self.low_fee_rate)
    ...
    def test_mixed_feerate_unconfirmed_parents(self):
        ...
        parent_low_txid = wallet.sendtoaddress(address=wallet.getnewaddress(), amount=1.5, fee_rate=self.low_fee_rate)
    ...
    def test_chain_of_high_low(self):
        ...
        parent_txid = wallet.sendtoaddress(address=wallet.getnewaddress(), amount=1.5, fee_rate=self.low_fee_rate)
    ...
    def test_chain_of_high_low_below_target_feerate(self):
        ...
        grandparent_txid = wallet.sendtoaddress(address=wallet.getnewaddress(), amount=1.8, fee_rate=self.mid_fee_rate)
        gp_tx = wallet.gettransaction(txid=grandparent_txid, verbose=True)
    ...
    def test_preset_input_cpfp(self):
        ...
        ancestor_aware_txid = wallet.send(outputs=[{self.def_wallet.getnewaddress(): 0.5}], options={"add_inputs": True, "fee_rate": self.target_fee_rate, "inputs": [{"txid": parent_txid, "vout": 0}]})["txid"]
        ancestor_aware_tx = wallet.gettransaction(txid=ancestor_aware_txid, verbose=True)
    ...
    def test_sibling_tx_gets_ignored(self):
        ...
        parent_txid = wallet.sendtoaddress(address=wallet.getnewaddress(), amount=1, fee_rate=self.mid_fee_rate)
        parent_tx = wallet.gettransaction(txid=parent_txid, verbose=True)

        self.assert_undershoots_target(parent_tx)

        # create sibling tx
        sibling_txid = wallet.sendtoaddress(address=self.def_wallet.getnewaddress(), amount=0.9, fee_rate=self.low_fee_rate)
    ...
    def test_sibling_tx_bumps_parent(self):
        ...
        # …because sibling bumped the parent above the target independently.
    ...
    def test_external_input_unconfirmed_low(self):
        ...
        parent_txid = wallet.sendtoaddress(address=external_address, amount=1, fee_rate=self.low_fee_rate)
        parent_tx = wallet.gettransaction(txid=parent_txid, verbose=True)

        self.assert_undershoots_target(parent_tx)

        spend_res = wallet.send(outputs=[{self.def_wallet.getnewaddress(): 0.5}], options={"fee_rate": self.target_fee_rate, "inputs":[{"txid":parent_txid, "vout":find_vout_for_address(self.nodes[0], parent_txid, external_address)}], "solving_data":{"descriptors":[external_descriptor]}})
    ...
    def run_test(self):
        ...
        self.low_fee_rate = 100
        self.mid_fee_rate = 150
        self.def_wallet  = self.nodes[0].get_wallet_rpc(self.default_wallet_name)
        ...
        self.log.info("Skipping RBF bumping: QQ/Blackcoin does not expose the inherited bumpfee RPC")
